...
void IRrecv0::blink13(int blinkflag)
{
...
int IRrecv0::decode(decode_results *results) {
...
long IRrecv0::decodeNEC(decode_results *results) {
  long data = 0;
...
long IRrecv0::decodeSony(decode_results *results) {
  long data = 0;
...
long IRrecv0::decodeSanyo(decode_results *results) {
  long data = 0;
...
// Looks like Sony except for timings, 48 chars of data and time/space different
long IRrecv0::decodeMitsubishi(decode_results *results) {
...
int IRrecv0::getRClevel(decode_results *results, int *offset, int *used, int t1) {
...
long IRrecv0::decodeRC5(decode_results *results) {
...
long IRrecv0::decodeRC6(decode_results *results) {
...
long IRrecv0::decodePanasonic(decode_results *results) {
...
long IRrecv0::decodeJVC(decode_results *results) {
    long data = 0;
    int offset = 1; // Skip first space
    // Check for repeat
...
long IRrecv0::decodeSAMSUNG(decode_results *results) {
  long data = 0;
  int offset = 1; // Skip first space
  // Initial mark
...
int IRrecv0::compare(unsigned int oldval, unsigned int newval) {
  if (newval < oldval * .8) {
    return 0;
  }
 ...
 */
long IRrecv0::decodeHash(decode_results *results) {
